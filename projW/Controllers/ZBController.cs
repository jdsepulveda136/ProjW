...
namespace projW.Controllers
{
    public class ZBController : Controller
    {
        ...
        // GET: ZB
        public ActionResult Index(int? tarefa)
        {
            var tLinhasDeTarefas = db.TLinhasDeTarefas.Include(l => l.Tarefa);

            if (tarefa.HasValue)
            {
                tLinhasDeTarefas = db.TLinhasDeTarefas.Where(t => t.TarefaId == tarefa);
            }

            if (tarefa==0)
            {
                tLinhasDeTarefas = db.TLinhasDeTarefas.Include(l => l.Tarefa);
            }
